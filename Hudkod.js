...
/* ------------------ функции подписи и хранения пользователя ------------------ */
...
/* ------------------ автологин и проверка подписки ------------------ */
document.addEventListener("DOMContentLoaded", async () => {
    try {
        const user = getHudUser();
        if (!user) {
            console.log("Пользователь не найден, показываем окно авторизации...");
            showAuthWindow();
            return;
        }

        const validSig = await verifyHudUserSignature();
        if (!validSig) {
            console.log("Подпись неверна, показываем окно авторизации...");
            showAuthWindow();
            return;
        }

        console.log("Пользователь найден, проверяем подписку...");

        const resp = await fetch(`${TOKEN_SERVER}/check-subscription`, {
            method: "POST",
        ...
        if (!data?.ok || !data?.active) {
            console.log("Подписка не активна, показываем окно авторизации...");
            showAuthWindow();
            return;
        }

        console.log("✅ Подписка активна — запускаем HUD");
        await initHUD();
...
async function showAuthWindow() {
    ...
    loginBtn.style.cssText = "width:100%; padding:10px; margin-top:10px; cursor:pointer;";

    const statusEl = document.createElement("div");
    statusEl.style.cssText = "margin-top:10px; font-size:13px; color:#FFD60A; min-height:18px; text-align:center;";

    box.append(closeBtn, title, userIn, passIn, loginBtn, statusEl);
