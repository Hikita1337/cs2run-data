...
// 📜 ЛОГИ HUD
// ===============================
const LOGS_KEY = "cs2run_hud_logs_v1";
...
function saveLogs(arr) {
  localStorage.setItem(LOGS_KEY, JSON.stringify(arr.slice(-300))); // храним максимум 300 строк
}
...
addLog("📡 Ably подключён, канал 'cs2run' открыт");
...
logBtn.textContent = "🪵";
logBtn.title = "Показать логи";
logBtn.style.cursor = "pointer";
logBtn.style.fontSize = "17px";
logBtn.style.userSelect = "none";
logBtn.onclick = openLogsModal;
rightControls.prepend(logBtn);
...
addLog(`✅ Участие в розыгрыше принято (lotteryId=${lotteryId})`);
  ...
  return true;
      ...
      }addLog(`⚠️ Ошибка при попытке участия: ${err.message || err}`);
    } catch (err) {
...
function openLogsModal() {
  ...
  header.style.justifyContent = "space-between";
  header.style.alignItems = "center";
  header.innerHTML = `<div style="font-weight:700">📜 Логи HUD</div>`;
  ...
  clearBtn.style.cssText = baseBtnStyle + "background:#FF3B30;color:#fff;";
  clearBtn.onclick = () => {
...
addLog("✅ HUD полностью загружен и активен");
